...
class ClientsServer {
    ...
    _setBotHandlers() {
        this._registerCommandHandler(BOT_COMMANDS.START, this._handleHelp.bind(this));
    ...
    _registerCommandHandler(command, handler) {
        this._telegramBot.onText(this._buildRegExpForBotCommand(command), async (message, match) => {
            ...
            try {
                ...
                await this._telegramBot.sendMessage(chatId, response, options);
            } catch (error) {
    ...
    async _handleHelp() {
        return messages.help({ domainName: this._domainName });
    }
