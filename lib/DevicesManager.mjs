...
import { generateRandomString } from './utils/random.mjs';
...
class DevicesManager {
    ...
    async markAsAlive({ deviceId, deviceToken }) {
        ...
        if (!device) {
            throw new Error('@REMOVE Not found');
        }

        if (device.token !== deviceToken) {
            throw new Error('@REMOVE Invalid token');
        }

        const wasDead = !device.alive;

        await this._devicesRepository.updateDevice(device.id, { alive: true, aliveAt: Date.now() });

        if (wasDead) {
            this._onBecomeAliveHandler({ clients: device.subscribers, deviceName: device.name });
        }
    ...
    async unsubscribeClientFromDevice({ client, deviceId }) {
        ...
        if (subscribers.length === device.subscribers.length) {
            throw new Error('@REMOVE Not subscribed');
        }
    ...
    _startHealthCheckInterval() {
        const checkFunction = async () => {
            ...
            for (const device of devices) {
                this._onBecomeDeadHandler({ clients: device.subscribers, deviceName: device.name });
            }
        };
